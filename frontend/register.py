...
class Register_Page:
    ...
    def add_click(self):
        ...
        if username == '' or password == '' or add == '' or gender == '' or phone == '':
            messagebox.showerror('Error', 'plz fill the empty field')
            return

        u = model.user.User(username, password, add, gender, phone)

        query = "insert into user_tbl(Username,Password,Address,Gender,phone) values(%s,%s,%s,%s,%s)"
        values = (u.get_username(), u.get_password(), u.get_address(), u.get_gender(), u.get_phone())
